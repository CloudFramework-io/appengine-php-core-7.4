...
shell_exec("cp -Ra vendor/cloudframework-io/appengine-php-core-7.4/install/api-dist/* api");

echo " - Copying /scripts examples\n";
if(!is_dir("./scripts")) mkdir('scripts');
shell_exec("cp -Ra vendor/cloudframework-io/appengine-php-core-7.4/install/scripts-dist/* scripts");

if(!is_file('./config.json')) {
    echo " - Copying composer.json\n";
    copy("vendor/cloudframework-io/appengine-php-core-7.4/install/config-dist.json", "./config.json");
...
if(!$replit) {
    ...
    copy("vendor/cloudframework-io/appengine-php-core-7.4/install/composer-dist.json", "./composer.json");

    if(!is_file('./app.yaml')) {
        echo " - Copying app.yaml for GCP appengine\n";
        copy("vendor/cloudframework-io/appengine-php-core-7.4/install/app-dist.yaml", "./app.yaml");
    } else echo " - Already exist app.yaml\n";

    if(!is_file('./.gcloudignore')) {
        echo " - Copying .gcloudignore\n";
        copy("vendor/cloudframework-io/appengine-php-core-7.4/install/.gcloudignore", "./.gcloudignore");
    } else echo " - Already exist .gcloudignore\n";

    if(!is_file('./README.md')) {
        echo " - Copying README.md\n";
        copy("vendor/cloudframework-io/appengine-php-core-7.4/install/README-dist.md", "./README.md");
    } else echo " - Already exist README.md\n";

    if(!is_file('./php.ini')) {
        echo " - Copying php.ini\n";
        copy("vendor/cloudframework-io/appengine-php-core-7.4/install/php-dist.ini", "./php.ini");
